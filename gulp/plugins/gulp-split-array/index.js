...
module.exports = function (jsonpath, itemname, itemnameextended = null) {
  function split(file, enc, cb) {
    ...
    if (!Array.isArray(jsonarray)) {
      return cb(
        new PluginError(
          PLUGIN_NAME,
          "Json at " + jsonpath + " does not contain an array."
        )
      );
    }
    ...
    for (var i = 0; i < jsonarray.length; i++) {
      ...
      if (itemnameextended) {
        let slugBaseExt =
          typeof item[itemnameextended] === "string"
            ? item[itemnameextended]
            : item[itemnameextended].toString();
        if (slugBaseExt) slugBase += "_" + slugBaseExt;
      }
      if (itemname)
        filename = slugify(slugBase, { lower: true }) + file.extname;
      var opts = {
